...
export default function QuestionFlow() {
  ...
  if (loading) return <div className="text-center p-8">Loading...</div>;
  if (error) return <div className="text-red-500 p-8">Error: {error}</div>;
  if (!currentQuestion || !Array.isArray(currentQuestion.QuestionOptions)) {
    return <div className="text-center p-8">No question loaded.</div>;
  }
              ...
              currentQuestion.QuestionOptions.map(option => (
                <button
                  ...
                  className={`w-full p-3 text-left rounded-lg transition-all
                    ...
                    hover:border-blue-300 text-gray-700 hover:text-blue-700
                    focus:outline-none focus:ring-2 focus:ring-blue-500
                    ${answers[currentQuestion.key] === option.value ? 'bg-blue-100 border-blue-500' : ''}`}
                >
                  {option.text}
                </button>
              ))}
              ...
              currentQuestion.QuestionOptions.map(option => (
                <div key={option.id} className="flex items-center">
                  <input
                    ...
                    className="h-5 w-5 text-blue-600 rounded"
                  />
                  <label htmlFor={`${currentQuestion.id}-${option.id}`} className="ml-2 text-gray-700">
                    {option.text}
                  </label>
                </div>
              ))}
